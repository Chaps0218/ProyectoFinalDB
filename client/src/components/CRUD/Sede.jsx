...
import { extraerSede, agregarSede, editarSede, eliminarSede } from "../../api/contratacion";
...
export default function App() {
  ...
  useEffect(() => {
    const fetchData = () => {
      extraerSede()
        .then((response) => {
          const contratoData = response.data.sede;
  ...
  const handleAgregar = React.useCallback(() => {
    ...
    const newActividad = {
      sede_nombre: nombreA,
      sede_descripcion: descripcion,
    };
    agregarSede(newActividad);
    ...
    return (
      <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-3">
            ...
            >
              <ModalContent>
                {(onClose) => (
                  <>
                    <ModalHeader className="flex flex-col gap-1">Agregar sede</ModalHeader>
                    <ModalBody>
        ...
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">Total {actividad.length} sedes</span>
  ...
  >
    ...
    <ModalContent>
      {(onClose) => ( 
        <>
          <ModalHeader className="flex flex-col gap-1">Actualizar sede</ModalHeader>
          <ModalBody>
          <div className="flex flex-wrap gap-8">
                <div className="w-full">
                    <Input
